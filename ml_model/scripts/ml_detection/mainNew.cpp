...
void print_partialCars(Camera& camera) {

    for (auto &frame : camera.xPoints) {
    
        cout << "frame: " << frame.first << endl;
        sort(frame.second.begin(), frame.second.end());
        for (auto &x : frame.second) {
            cout << "\t\tx point: " << x << endl;
        }
    }
/*
    for (auto &car: camera.partialCars) {
        cout << "car id: " << car.id << endl;
        if (car.frontBack == 1) {
            cout << "front wheel x coordinate: " << car.front.circle[0] << endl;
        }
        else {
            if (car.frontBack == 2) {
                cout << "back wheel x coordinate: " << car.back.circle[0] << endl;
            }
            else {
                cout << "front wheel x coordinate: " << car.front.circle[0] << endl;
                cout << "back wheel x coordinate: " << car.back.circle[0] << endl; 
...
int main(int argc, const char * argv[]) {
    ...
    VideoCapture reader("test_data/video6x.5spd1.mp4");
...
//    reader.set(1, 60);
    ...
    for (;count < frameCount; ++count) 
    {
        ...
        if (PRINTFRAMES) imshow("blur image", imageBlur);
        ...
        if (PRINTFRAMES) imshow("threshold image", imageThresh); 

        tracker->update(imageThresh, wheelList, camera, count);
        ...
        if (PRINTFRAMES) imshow("Tracking", frame);
        if (WRITEVIDEO) {
