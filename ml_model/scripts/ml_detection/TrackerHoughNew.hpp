...
public:
    ...
    CV_WRAP bool update( InputArray image, CV_OUT std::vector<Wheel>& wheelList, Camera& camera, int frame);
    
    virtual ~TrackerHough();
    
protected:
    virtual bool initImpl( const Mat& image, const std::vector<Wheel>& wheelList ) = 0;
    virtual bool updateImpl( const Mat& image, std::vector<Wheel>& wheelList, Camera& camera, int frame) = 0;
    bool isInit;
};
...
public:
    ...
    bool updateImpl(const Mat& image, std::vector<Wheel>& wheelList, Camera& camera, int frame);
...
private:
    ...
    /**
     ...
     @return true or false
     */
    bool updateNewWheel(const cv::Vec3f &circle, std::vector<Wheel>& wheelList, Camera &camera, int frame);
